...
class RAGEvaluator:
	...
	def __init__(
		...
		self._evaluation_llm_model   = evaluation_llm_model
	...
	async def run(self, cases: list[TestCase]) -> RAGReport:
		...
		llm        = ChatOllama(model=self._evaluation_llm_model, base_url=self._ollama_url, num_predict=2048, format='json', temperature=0)
